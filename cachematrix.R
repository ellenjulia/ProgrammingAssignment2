## The following two functions work in tandem to streamline repeated uses of a matrix 
## inverse. The makeCacheMatrix function creates a list object that enables the caching of the 
## inverse of a matrix once it has been computed so that repeated computation of the inverse is 
## not necessary. The cacheSolve function, which takes the list returned by makeCacheMatrix as 
## its argument, first determines whether the inverse has been previously cached, and then either 
## returns the cached value or computes the inverse and stores this value in the list for later use. 
